...
In image classification, an image is assigned to a predefined set of classes. In multi-class classification, there are ≥2 classes; in binary classification, there are 2 classes; and in multi-label classification, there are multiple sets of classes. @fig-image-classifier-example1 shows an example from the paper by @krizhevsky_imagenet_2012 (multi-class classification), which achieved the best results in the ImageNet competition in 2012 and demonstrated the effectiveness of CNNs.

::: {#fig-image-classifier-example1}

![]({{< meta params.images_path >}}image_classification_example.png){width=600}


Multi-Class Image Classification example (from @krizhevsky_imagenet_2012).
:::

### Object Detection

Object detection involves locating and recognizing (multiple) objects in an image. @fig-intro-yolo-example shows an example from the paper by @redmon_you_2016. Each object is localized with a bounding box and assigned an object class.
...
Object Detection example (from @redmon_you_2016). Bounding boxes localize the objects, indicating the most likely class and confidence for each object.
:::
...
@fig-pix2pix-example shows an example of image generation from the paper by @image_to_image_isola2018, where images are generated conditioned on image inputs (translated).

::: {#fig-pix2pix-example}

![]({{< meta params.images_path >}}pix2pix_examples.png){width=600}
...
![]({{< meta params.images_path >}}dssl.png){width=600}
...
![]({{< meta params.images_path >}}nerf.png){width=600}
...
![]({{< meta params.images_path >}}stylegan3_examples.png){width=600}


StyleGan3 (from @karras_alias-free_2021).
:::

@fig-intro-draggan shows how generated images can be further adjusted with manually set reference points.

::: {#fig-intro-draggan}

![]({{< meta params.images_path >}}drag_gan.png){width=600}


DragGAN (from @pan_drag_2023).
:::
...
CNNs are still in use today (2024). Meanwhile, there are alternative architectures, such as transformer-based models @dosovitskiy_image_2020, which are extremely successful in language modeling, or multilayer perceptron-based architectures @liu_pay_2021. However, it has been shown that CNNs are still competitive and sometimes superior to alternative architectures @woo_convnext_2023. The question of which architecture type will prevail is open. Currently, it seems that CNNs and transformer-based models perform similarily @smith_convnets_2023.
